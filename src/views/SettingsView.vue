...
import { cancelFeedArchive, deleteAllAppData, deleteCurrentAccountData, getArchiveProgress, getPrivacyStatus, type PrivacyStatus } from "../utils/qzone";
...
async function stopArchiveBeforeDelete() {
  try {
    let progress = await getArchiveProgress();
    if (progress.status !== "running") return;
    await cancelFeedArchive();
    for (let attempt = 0; attempt < 30; attempt += 1) {
      await delay(200);
      progress = await getArchiveProgress();
      if (progress.status !== "running") return;
    }
    throw new Error("归档任务仍在结束中，请稍后重试删除");
  } catch (reason) {
...
<template>
  ...
  <Dialog v-model:visible="privacyVisible" modal :draggable="false" class="privacy-dialog" header="隐私与数据处理">
    <div class="privacy-content">
      ...
      <h4>1. QQ 登录凭证</h4><p>扫码登录产生的 Cookie、p_skey、skey、g_tk 等仅保存在 Rust 后端内存中，不返回前端、不写入日志、不写入磁盘。退出程序或退出登录后会话即被清除。</p>
      ...
      <h4>4. 数据清除</h4><p>可以删除当前 QQ 的本地记录，也可以删除全部本地数据。删除当前 QQ 时会启用 SQLite 安全删除、截断 WAL 并压缩数据库；由于上游历史版本的媒体缓存目录是共享的，也会同时清空媒体缓存，避免图片或视频残留。</p>
  ...
  <Dialog v-model:visible="deleteCurrentVisible" modal :closable="!deletingCurrent" :draggable="false" class="delete-dialog" header="删除当前 QQ 的本地数据？">
    <div class="delete-dialog-content"><span class="delete-warning"><i class="pi pi-exclamation-triangle" /></span><div><p>QQ {{ user?.uin }} 的归档、互动、续传和异常记录将被永久删除，并退出当前登录。</p><small>如归档任务正在运行，软件会先安全停止任务。为确保没有媒体残留，共享图片/视频缓存也会清空；其他 QQ 的数据库记录不会删除，需要时可重新加载媒体。</small></div></div>
  ...
  <Dialog v-model:visible="deleteVisible" modal :closable="!deleting" :draggable="false" class="delete-dialog" header="删除所有数据？">
    <div class="delete-dialog-content"><span class="delete-warning"><i class="pi pi-exclamation-triangle" /></span><div><p>所有账号的本地归档和媒体缓存都将被永久删除。</p><small>包括动态、留言、评论、点赞、续传记录、视频缓存及内存中的登录状态。若归档正在运行，软件会先安全停止任务。此操作无法撤销。</small></div></div>
