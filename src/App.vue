...
<template>
  ...
  <Dialog :visible="!disclaimerAccepted" modal :closable="false" :draggable="false" :close-on-escape="false" class="disclaimer-dialog" header="免责声明与使用须知">
    <div class="disclaimer-heading"><span><i class="pi pi-shield" /></span><div><strong>请在使用恢复归档前仔细阅读</strong><small>协议版本：2026-08-28 · 只有同意后才能进入应用</small></div></div>
    <div class="disclaimer-content">
      ...
      <section><h4>二、授权范围与账号责任</h4><p>你确认仅使用本人账号，或已获得账号所有人及相关内容权利人的合法、充分授权。你应妥善保管设备、二维码、导出文件和缓存，不得出借账号、冒用身份或允许未经授权者访问。因账号共享、设备遗失、恶意软件、系统越权或保管不当造成的风险由相应责任方依法承担。</p></section>
      ...
      <section><h4>六、本地存储与网络请求</h4><p>归档数据库和媒体缓存保存在当前设备本地；QQ Cookie、p_skey、skey、g_tk 等登录凭证只存在于 Rust 后端内存，不返回前端、不写入日志、不持久化到磁盘。登录、读取资料和下载媒体会直接连接腾讯相关服务，不经过开发者自建中转服务器。本软件不接入遥测、广告追踪或第三方崩溃上报。</p></section>
      <section><h4>七、数据丢失与备份风险</h4><p>软件可能因接口变更、临时签名过期、网络中断、限流、账号权限、数据库损坏、设备故障、系统清理、存储不足、升级卸载或误操作出现遗漏、重复、乱码、媒体失效或数据丢失。归档结果不构成唯一、永久或完整备份。请定期核验并将重要资料备份到安全位置；执行数据删除前应确认已完成必要备份。</p></section>
